...
[ TemplateCategorySection — 结构模板库（按 category 聚合，真实样例封面） ]
...
### 5.2.1 结构模板库（2026-06-09）

**计划 / UI SSOT：** [knowledge-category-template-bootstrap-plan.md](../plans/2026-06-09-knowledge-category-template-bootstrap-plan.md)、[knowledge-template-ui-spec.md](./2026-06-09-knowledge-template-ui-spec.md)

**路由：** 首页区块 `/projects`；详情 `/templates/{categorySlug}`

**美学：** Editorial Template Shelf — 用 **真实 keyframe poster** 作封面，与创意库 hash 渐变区分；不展示 full skill Markdown。
...
**选用态：** 主 entry `border-primary`；参考 entry `border-ai/40` + Badge；上限 1 主 + 2 参考。创建项目后 **跳转工作台**（不在详情页上传）。

### 5.3 工作台 (`/projects/{id}`)

工作台采用 **三阶段 Stepper + 分组 Tab + 上下文 CTA**，替代原先「功能 Card 平铺」导航。视觉继承全局 tokens（Warm Creative Studio）。

#### 顶区布局

```
┌─────────────────────────────────────────────────────────────┐
│ 项目名称 (serif)                    [次 CTA] [主 CTA]        │
│ 可解释结构迁移 · proj-id (mono)                              │
│ 当前样例：sample-id (mono, 可选)                             │
├─────────────────────────────────────────────────────────────┤
│ Stepper: ① 准备 ── ② 生成 ── ③ 产出                         │
├─────────────────────────────────────────────────────────────┤
│ Tab 组（按当前 phase 高亮；非当前 phase Tab 降低 opacity）    │
│ 准备: 录入 | 样例分析 | 知识库                               │
│ 生成: 进度 | 脚本审核                                        │
│ 产出: 缺口 | 时间线 | 全片口播 | 结果                        │
└─────────────────────────────────────────────────────────────┘
...
**上下文主 CTA（`ProjectWorkbench` 顶栏右侧）：**

| 状态 | 主按钮 | 次按钮 |
|------|--------|--------|
| 无 sample | 禁用「开始样例分析」 | 加载演示数据 (outline) |
| 有 sample、未分析 | **开始样例分析** | 开始生成视频 (disabled) |
| 有已分析样例 | 开始样例分析 (outline) | **开始生成视频** |
| 任务进行中 | **查看进度** | — |

实现：`WorkbenchStepper.tsx`、`workbenchPhases.ts`。

#### 录入视图 — Input Wizard

单栏 `max-w-6xl mx-auto`；**lg+** Step 1 与 Step 2 并排（`InputWizardPrimaryGrid`）；Step 1 内 **lg+** 上传区与样例 filmstrip 列表左右分栏；Advanced 样例选择/知识推荐 **xl+** 并排。

```
Step 1 · 样例视频
  └ filmstrip 列表 (SampleVideoCard variant="filmstrip")
  └ Dropzone + 「分析最近批次」(outline sm)
  └ 点击缩略图 → SamplePreviewDialog

Step 2 · 创作 Brief
  └ Tier1 常显: 内容类型 (select) / 主题 / 目标 / 核心信息点
  └ Tier2 折叠: 主体 / 受众 / 语气 / 补充说明
  └ Tier3 折叠: 必须/禁止提及
  └ header 右侧 sticky「保存 Brief」

Step 3 · 素材与生成配置 (默认折叠)
  └ GenerationConfigPanel: 目标时长 + 变体 checkbox + AssetInputPanel

Advanced · 样例选择 / 知识推荐
  └ 无已分析样例 / 无知识条目 → 一行 muted 提示
  └ 有条目时紧凑 UI，规则收进 tooltip
```

Section 壳：`rounded-2xl border bg-card shadow-sm`；Step 序号 `text-xs uppercase tracking-wide text-muted-foreground`；Step 标题 `font-serif text-lg font-semibold`。

#### 样例分析视图 — Timeline + Master-Detail

网格 `xl:grid-cols-[240px_1fr]`：左侧 56px 紧凑样例列表；右侧 Master-Detail。

```
StructureHero (摘要 + 元数据 + 音频 badge + promote CTA)
NarrativeTimeline (水平比例条, ring-ai 选中, hover 关键帧)
SegmentDetailPanel (段证据 + RelatedSlotsColumn)
<details> 四轨深度洞察 (StructureV3TrackPanel, 默认折叠)
```

侧栏：点击整行切换样例；**无**内嵌 video player、**无**冗余 Banner。

#### 组件索引

| 组件 | 路径 |
|------|------|
| WorkbenchStepper | `apps/web/features/workbench/WorkbenchStepper.tsx` |
| Input Wizard | `apps/web/features/project-input/InputWizardLayout.tsx` |
| Structure Hero / Timeline | `apps/web/features/structure-evidence/StructureHero.tsx`, `NarrativeTimeline.tsx` |
| 样例预览 | `apps/web/components/sample-preview-dialog.tsx` |
...
| 结构模板库 | `apps/web/components/home/template-category-section.tsx`；UI 规范 `docs/superpowers/specs/2026-06-09-knowledge-template-ui-spec.md` |
| 模板详情 | `apps/web/app/templates/[categorySlug]/page.tsx` |
...
| 2026-06-09 | **结构模板库 UI**：§5.2.1 首页 TemplateCategorySection + `/templates/[categorySlug]` 详情；Editorial Template Shelf；独立 UI 规范 |
