# Agents Reference (detailed)

> Encyclopedic companion to the slim root [`AGENTS.md`](../../AGENTS.md).
> Use **AGENTS.md** for session defaults (module map, hard rules, verification).
> Use **this file** for plan indexes, API/route catalogs, env tables, and subsystem prose.
>
> Migrated from the former monolithic `AGENTS.md` (2026-07-15). Env defaults: also `services/api/.env.example`.
...
**Day-to-day agent contract:** keep using [`AGENTS.md`](../../AGENTS.md). This document holds the long-form material that used to live there.
...
| `2026-06-10-poster-cover-pipeline-plan.md` | 独立 `poster.jpg` 封面管线（upload/analyze/render 写入；项目/模板库统一读取） | `docs/demos/knowledge-category-template-e2e-checklist.md` § Poster |
...
| `2026-07-06-acp-agent-hardening-plan.md` | ACP 硬化：审片 cap=1 + repair、禁止辅助脚本、MCP bootstrap | `docs/demos/material-review-gate-e2e-checklist.md` § ACP 硬化 |
| `2026-07-06-gate-revise-latency-plan.md` | Gate NL 改片热路径、lint 收敛、preview 一致性；保留 `ACp_IN_SESSION_REVISE` | `docs/demos/material-review-gate-e2e-checklist.md` § Gate revise 降耗 |
...
| `VIDEOMAKER_MATERIAL_REVIEW_MAX_ROUNDS` | Max billed vision reviews per slot (ACP worker post-turn; ReAct `review_material_preview` tool) | `1` |
| `VIDEOMAKER_MATERIAL_REVIEW_REPAIR_FOLLOWUP_MAX` | Max review-driven repair follow-ups per ACP session when vision cap reached (no second billed review) | `1` |
| `VIDEOMAKER_MATERIAL_REVIEW_ACP_IN_SESSION` | Worker post-turn vision on first-generation ACP author | `true` |
| `VIDEOMAKER_MATERIAL_REVIEW_ACP_IN_SESSION_REVISE` | Worker post-turn vision on gate NL revise / scoped ACP author | `true` |
| `VIDEOMAKER_MATERIAL_REVIEW_GATE_LLM` | When in-session review is off and no review marker, run `material_reviewer` at gate finalize | `true` |
| `VIDEOMAKER_MATERIAL_PREVIEW_PROFILE` | In-session material preview HF render: `full` or `fast` (`draft` quality) | `full` |
| `VIDEOMAKER_COMPOSITION_ACP_DIALOGUE_SAFETY_MAX` | Max ACP `conn.prompt()` rounds per session (anti-dead-loop; not lint/review budget) | `15` |
| `VIDEOMAKER_COMPOSITION_ACP_MAX_TURNS` | **Deprecated** — fallback alias for `DIALOGUE_SAFETY_MAX` only | — |
| `VIDEOMAKER_ACP_SPAWN_CWD` | ACP agent spawn cwd: `scratch` or `repo` | `scratch` |
...
| `VIDEOMAKER_COMPOSITION_REACT_MAX_TURNS` | Max ReAct **LLM orchestration** turns (orthogonal to `MATERIAL_REVIEW_MAX_ROUNDS`; not used by ACP) | `12` |
...
**Session workflow, module map, and hard rules:** [`AGENTS.md`](../../AGENTS.md).  
**Composition author on-disk debug:** [`.cursor/skills/composition-agent-debug/SKILL.md`](../../.cursor/skills/composition-agent-debug/SKILL.md).
