...
def test_resolve_asset_understanding_route_defaults_legacy(store: ModelGatewayStore) -> None:
    assert resolve_asset_understanding_route(store) == "legacy"


def test_resolve_asset_understanding_route_direct_when_configured(
    ...
    assert resolve_asset_understanding_route(store) == "direct_multimodal"


def test_resolve_asset_understanding_route_respects_preference(store: ModelGatewayStore) -> None:
    store.update_providers(
        {
            "videoUnderstanding": {
                "baseUrl": "https://ark.cn-beijing.volces.com/api/v3",
