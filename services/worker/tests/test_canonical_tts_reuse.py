...
def test_try_reuse_canonical_master_wav_copies_when_hash_matches(tmp_path: Path) -> None:
    ctx = _ctx(tmp_path)
    generation_root = ctx.project_root / "generations" / ctx.generation_id
    draft = {
        "masterNarration": "测试口播。",
        "narrationVoProfile": None,
        "storyboard": ctx.storyboard,
    }
    _write_wav(generation_root / "narration" / "canonical.wav")
    ...
    assert try_reuse_canonical_master_wav(ctx, output)
    assert output.is_file()
    assert output.stat().st_size > 0


def test_try_reuse_canonical_master_wav_skips_when_hash_differs(tmp_path: Path) -> None:
    ctx = _ctx(tmp_path)
    generation_root = ctx.project_root / "generations" / ctx.generation_id
    _write_wav(generation_root / "narration" / "canonical.wav")
    save_narration_timing(
    ...
    output = ctx.generated_root / "master.wav"
    assert not try_reuse_canonical_master_wav(ctx, output)
