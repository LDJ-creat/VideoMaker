...
def visual_craft_bootstrap_section() -> str:
    return "\n".join(
        [
            ...
            "Priority: visualStyleBible.avoid > visualStyleBible > videomaker-visual-craft.",
            "No purple-pink diagonal gradients; no left-border accent cards.",
            "Required skill_view before submit:",
            ...
            f"  + {VISUAL_BIBLE_EXTRA_READ_PATHS[0]} if visualStyleBible present",
            "styles: :root { --vm-bg; --vm-fg; --vm-accent; --vm-muted; }; use var(--vm-*).",
            "≥1 content-driven motion beat; hold final frame through slotTiming.durationSec.",
            "",
            "# Copy policy",
            "Implement authorPrompt as layout/motion — never render brief/VO verbatim.",
            "Only renderPolicy/authorContract allowedDisplayCopy may appear as readable Chinese.",
            "Paste allowlisted strings exactly; highlight keywords with spans inside full allowed lines.",
        ]
    )
